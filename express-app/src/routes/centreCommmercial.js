...
const Centre = require('../models/CentreCommmercial');
...
router.delete('/:id', async (req,res)=>{ try{ const d=await Centre.findByIdAndDelete(req.params.id).exec(); if(!d) return res.status(404).json({status:404,message:'Not found'}); res.status(200).json({status:200,message:'Centre deleted successfully',data:null}); }catch(err){res.status(500).json({status:500,message:err.message});}});
